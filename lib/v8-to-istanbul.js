...
module.exports = class V8ToIstanbul {
  ...
  async load () {
    ...
    if (rawSourceMap) {
      if (rawSourceMap.sourcemap.sources.length > 1) {
        ...
        let useParser = undefined;
        if (this.path.slice(-3) === ".ts") {
          useParser = "tsestree";
        }

        this.source = new CovSource(originalRawSource, this.wrapperLength, useParser)
  ...
  _statementsToIstanbul () {
    ...
    this.source.lines.forEach((line, index) => {
      let ignore = false;
      if (this.source.ignoreSegments.some(([start, end]) => line.startCol >= start && line.endCol <= end)) {
        ignore = true;
      }
      statements.statementMap[`${index}`] = line.toIstanbul()
      statements.s[`${index}`] = Math.max(line.count, ignore ? 1 : 0)
    })
    return statements
  }
  ...
  _branchesToIstanbul () {
    ...
    this.branches.forEach((branch, index) => {
      let ignore = this.source.lines[branch.startLine - 1].ignore
      if (this.source.ignoreSegments.some(([start, end]) => branch.absoluteStartCol >= start && branch.absoluteEndCol <= end)) {
        ignore = true;
      }
      branches.branchMap[`${index}`] = branch.toIstanbul()
      branches.b[`${index}`] = [Math.max(branch.count, ignore ? 1 : 0)]
    })
    return branches
  }
  ...
  _functionsToIstanbul () {
    ...
    this.functions.forEach((fn, index) => {
      let ignore = this.source.lines[fn.startLine - 1].ignore
      if (this.source.ignoreSegments.some(([start, end]) => fn.absoluteStartCol >= start && fn.absoluteEndCol <= end)) {
        ignore = true;
      }
      functions.fnMap[`${index}`] = fn.toIstanbul()
      functions.f[`${index}`] = Math.max(fn.count, ignore ? 1 : 0)
    })
    return functions
  }
}
